...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        //wybielanie i odchudzanie krawedzi po X
        BufferedImage image_white_edge_X = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++) {
                tab_edge_white_X[i][j] = (tab_edge_X[i][j].getRed()<50 && tab_edge_X[i][j].getGreen()<50 && tab_edge_X[i][j].getBlue()<50) ? new Color(0, 0, 0) : new Color (255, 255, 255);
        ...
        //wybielanie i odchudzanie krawedzi po Y
        BufferedImage image_white_edge_Y = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++) {
                tab_edge_white_Y[i][j] = (tab_edge_Y[i][j].getRed()<50 && tab_edge_Y[i][j].getGreen()<50 && tab_edge_Y[i][j].getBlue()<50) ? new Color(0, 0, 0) : new Color (255, 255, 255);
        ...
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++) {
                tab_edge_white[i][j] = (tab_edge_white_X[i][j].getRed() == 255 || tab_edge_white_Y[i][j].getRed() == 255) ? new Color(255, 255, 255) : new Color (0, 0, 0);
